...
pub async fn change_sg(username: String, command_argument: Option<String>) -> anyhow::Result<()> {
    let command_argument: CommandArgument =
        cmd::parse_arguments(command_argument.context("missing arguments")?)?;

    let response: Response =
        session::send_request(cmd::Request::ChangeSgForUser(cmd::RequestChangeSgForUser {
            username: username,
