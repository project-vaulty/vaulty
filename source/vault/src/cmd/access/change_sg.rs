...
pub async fn change_sg(
    ...
    data: cmd::RequestChangeSgForAccessKey,
) -> AppResult<()> {
    ...
    let response = match access_keys::change_sg(&data.vault, &data.access_key, data.sg)? {
        access_keys::ChangeSgForAccessKeyResult::Updated => Response {
            result: ResponseState::Updated,
        },
        access_keys::ChangeSgForAccessKeyResult::NotFound => Response {
    ...
    log!({
        ...
        ctx: "request to change access key's security group",
        msg: "access key's security group was changed",
        tags: [
